...
export const cartSlice = createSlice({
    ...
    reducers: {
        addItem:(state,action)=>{
            ...
            const indexItem = items.findIndex(item=>(item.id===newItem.id&&JSON.stringify(item.option)==JSON.stringify(newItem.option)))
            if(indexItem >= 0){
...
const findIndexItem = (arr,item)=>arr.findIndex(e=>(e.id===item.id&&JSON.stringify(item.option)==JSON.stringify(e.option))) 
