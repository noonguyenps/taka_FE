    ...
    useEffect(()=>{
      const handle = ()=>{
        if(!props.setAddressDetails)
          return
        let province = listprovince?.find(item=>item.code === props.province)
        let district = listdistrict?.find(item=>item.code === props.district)
        let commune = listcommune?.find(item=>item.code === props.commune)
  ...
  return (
    ...
    <Stack direction="row">
          ...
          <FormControl className="create-address__input" sx={{flex:"1"}}>
            ...
            >
              {
                listprovince.map(item => <MenuItem value={item.code}>{item.name}</MenuItem>)
              }
            </Select>
          </FormControl>
        </Stack>
        ...
        <Stack direction="row">
          ...
          <FormControl className="create-address__input" sx={{flex:"1"}}>
            ...
            >
              {
                listdistrict.map(item => <MenuItem value={item.code}>{item.name}</MenuItem>)
              }
            </Select>
          </FormControl>
        </Stack>
        ...
        <Stack direction="row">
          ...
          <FormControl className="create-address__input" sx={{flex:"1"}}>
            ...
            >
              {
                listcommune.map(item => <MenuItem value={item.code}>{item.name}</MenuItem>)
              }
            </Select>
          </FormControl>
        </Stack></>
